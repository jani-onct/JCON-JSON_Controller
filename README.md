...
| :----------------------- | :------------------------------------------------ | :------------------------------ |
| **ver1.0.0** | 2025/11/11 | 基本機能(JSONキー任意設定，ESP32との接続) |
...
| **ver1.2.1** | 2025/12/12 | 設定画面でゲームパッドのラベルを変更してもJSONのキーに反映されない問題を解決 |

**【アプリの特徴】**
* **カスタマイズ性**: 12個のコントロールスロット（ボタン, ジョイスティック, スライダー，十字キーなど）を自由に設定可能。
* **データ形式**: 軽量なJSON形式でデータを送信。マイコン側で簡単にパースできます。
* **安定接続**: デバイス名プレフィックス（`JCON-`）とサービスUUIDによる効率的なフィルタリング。
* **外部ゲームパッドサポート**：iPhoneに優先や無線で接続した外部ゲームパッドの値をマイコンボードに送信できます。

## サンプルファームソースコード

以下のリンクから、各マイコンボードで使用可能なファームウェアのサンプルコードをダウンロードできます。
 
* **サンプルコード(ESP32)**:https://github.com/jani-onct/JCON-JSON_Controller/tree/main/sample/JCON_ESP32
* **サンプルコード(Rapberry Pi Pico W)**:https://github.com/jani-onct/JCON-JSON_Controller/tree/main/sample/JCON_PicoW
...
各種設定を行う画面です。  
以下のような項目を設定できます。  
- 外観モードの設定(端末設定/ライト/ダーク)
- 送信データ設定(アプリ/ゲームパッド)
- JSONモニタ表示設定(非表示/送信表示/受信表示)
  ※送信表示と受信表示は操作画面でも切り替えできます。
- アクセントカラー
- BLE送信周期(更新周期)
- ゲームパッド設定(ラベル，Axesの最大値など)
- 各スロットに割り当てる機能(ボタン、ジョイスティック、スライダー)  
  
各スロットのラベルを編集することで、JSONのキーに反映されます．また、ジョイスティックとスライダーはその最大値を設定することができます．
